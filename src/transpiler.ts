...
function replaceComparisonOperators(text: string): string {
  let result = text;
...
function emit(node: any, indent: string = ""): string {
  ...
  if (node.type === "CaseBlockContent") {
    return node.lines ? node.lines.map((line: any) => emit(line, indent)).join("\n") 
                      : node.body ? node.body.map((line: any) => emit(line[0], indent)).join("\n")
                      : "";
  }
